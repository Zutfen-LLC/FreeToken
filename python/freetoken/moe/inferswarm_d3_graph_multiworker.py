...
class InferSwarmD3GraphMultiworkerExecutor:
 ...
 def decode(self,l,c,h,w,ids):
  ...
  try:
   ...
   for branch_index,x in enumerate(self.active_workers):
    getattr(self,f"done_{x}_events")[n].wait(s);getattr(self,f"gpu0_{x}_return_routes").copy_(getattr(self,f"host_{x}_return"),non_blocking=True)
    # Keep the two AB reconstruction writes disjoint: local + A, then reconstruction + B.
    destination=self.gpu0_reconstruction if branch_index==0 else self.gpu0_final_routes
    torch.add(routes,getattr(self,f"gpu0_{x}_return_routes"),out=destination);routes=destination
...
def absent_d3_graph_multiworker_report():return {"schema":D3_EXECUTOR_SCHEMA,"experimental":True,"enabled":False,"active_workers":[],"active_worker_count":0,"graph_active":False,"graph_topology":None,"captured_batch_sizes":[],"graph_replays_per_token":0,"eager_fallback":False,"fallback_count":0,"failure_count":0,"steady_state_host_sync_count":0}
