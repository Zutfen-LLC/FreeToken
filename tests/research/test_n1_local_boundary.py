...
def _hidden(tokens: int = 1) -> bytes:
    return bytes((index % 251 for index in range(tokens * BOUNDARY_PLANES * HIDDEN_SIZE * 2)))
...
def test_bf16_bytes_are_never_numeric_serialized():
    ...
    payload = bytes(range(256)) * (BOUNDARY_PLANES * HIDDEN_SIZE * 2 // 256)
...
def test_wrong_dtype_and_hidden_size_are_rejected():
    ...
    with pytest.raises(BoundaryProtocolError, match="hidden size"):
        encode_frame(Frame(MessageType.DECODE, 1, 0, 0, 1, 1, 1024, BF16_DTYPE_CODE, 0, _hidden()))
...
def test_handshake_round_trip_and_mismatch_fail_closed():
    expected = Handshake("repo", "r" * 40, "a" * 64, 0, 19, 19, 40, 2048, 1,
                         "GPU-a", "GPU-b")
    assert Handshake.decode(expected.encode()) == expected
    wrong = Handshake("repo", "x" * 40, "a" * 64, 0, 19, 19, 40, 2048, 1,
                      "GPU-a", "GPU-b")
