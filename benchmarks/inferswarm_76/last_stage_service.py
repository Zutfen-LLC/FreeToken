"""#76 remote last-stage service (inferswarm03) over the accepted R4 wire.

Identical framing, boundary contract, and execution semantics to the accepted
R6 ``last_stage_service``. Differences (all evidence-side, no execution math):

- per-CASE capture sinks (``RowPruningSink``) instead of one sink for the
  whole connection — the 584-case campaign cannot hold all captures in host
  RAM; each case's bundle is persisted at ``CASE_SAVE``;
- the o_proj/full-row capture wrappers from ``benchmarks.inferswarm_76``;
- ``--allow-producer`` records the #76 frozen producer explicitly (the plan
  file carries the historical R6 producer; #76 is an authorized execution
  campaign under a NEW frozen producer derived from the same base).

The service speaks two extra request ops beyond the R6 protocol:
    ...
    *,
    ...
    if plan_producer and running_sha != plan_producer:
        if allow_producer and allow_producer == running_sha:
            producer_check = {
                "mode": "EXPLICIT_OVERRIDE_ISSUE76_EXECUTION",
                "plan_frozen_producer": plan_producer,
                "running_producer": running_sha,
                "reason": "issue #76 authorized execution campaign under a "
            ...
            raise RuntimeError(
                ...
                f"{running_sha!r} for the #76 execution campaign"
            )
    else:
        producer_check = {
            "mode": "PLAN_FROZEN",
    ...
    def _arm(case_id: str) -> None:
        ...
        # stage owns globals [32,48): after_layer_47 is emitted explicitly by
        # the prefill path; the last stage owns no layer 0/15.

    # The last stage owns global layers [32,48); layer-0/15 wrappers do not
    # apply. arm_full_capture no-ops for absent layers.
    ...
    try:
        ...
        try:
            Path(os.environ.get(
                "I76_LAST_STAGE_FINAL_REPORT",
                "/tmp/i76-last-stage.json")).write_text(json.dumps({
                    "schema": "inferswarm.issue76.last-stage-final-report/1",
                    "plan_digest": plan.get("digest"),
                    "producer_freetoken_sha": running_sha,
                    "producer_check": producer_check,
                    "stats": stats,
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out-dir", required=True,
                        help="root dir for per-case capture bundles")
