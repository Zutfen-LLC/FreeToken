"""#110 v5 reference runner: RTX 3090 canonical reference execution.

Runs the frozen c109-*/p109-* case manifests through the accepted execution
path and retains the v5 semantic evidence per case:
...
- FULL decision evidence at ALL 8 decisions: the complete FP32
  ...
  sidecars, sha256-recorded) — exactly what the v5 comparator's
  all-8-decision capture_position_rule requires for the
  fp32-consumer-logits reducers and later decision_local_error derivation;
- the frozen decision domain D(r) per decision, computed on this reference
  row only, with canonical membership hashes;
...
The execution path is the accepted #97 reference runner, re-invoked with v5
identity bindings; ZERO execution math differs.  One process runs many
cases sequentially (model load dominates).  Nothing is ever overwritten.
"""
...
def main(argv=None) -> int:
    ...
    index = {
        "schema": REFERENCE_RUN_INDEX_SCHEMA,
        "contract_id": CONTRACT_ID,
        "subject": subject,
        "attempt_id": args.attempt_id,
        "producer": producer,
