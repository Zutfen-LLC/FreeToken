"""#110 v5 remote last-stage service (inferswarm03) over the accepted R4 wire.

Identical framing, boundary contract, and execution semantics to the #97
...
- at EVERY prefill the final-row FP32 consumer logits are retained as
  ``<out_dir>/<case_id>/decision-<capture_step>.f32`` (the #110 chain
...
- the external `--expected-producer-sha` is checked against a clean running
  ...
  only as explicit issue-110 override provenance.
...
def serve(
    *,
    ...
    from benchmarks.inferswarm_110 import frozen_subject_record, require_producer_identity
    ...
    producer_check = {
        "mode": "EXPLICIT_OVERRIDE_ISSUE110_EXECUTION" if plan_producer != producer["commit"] else "PLAN_FROZEN",
        "plan_frozen_producer": plan_producer,
        "running_producer": producer["commit"],
        "expected_producer": expected_producer_sha,
        "reason": "issue #110 externally pinned producer supersedes the historical R6 plan producer" if plan_producer != producer["commit"] else "plan producer matches the externally pinned issue #110 producer",
    }
    ...
    try:
        ...
        while True:
            ...
            # --- v5: retain EVERY decision's FP32 row + rule proof ------
            rule_proof = None
            row_sha = None
            row_count = None
            if header["operation"] == "prefill" and capture_step is not None:
                from benchmarks.inferswarm_110 import executor_rule_proof
                ...
                if current_case_dir is None:
                    raise WireError("decision row before CASE_SAVE/CASE_BEGIN")
        ...
        try:
            Path(os.environ.get(
                "I110_LAST_STAGE_FINAL_REPORT",
                "/tmp/i110-last-stage.json")).write_text(json.dumps({
                    "schema": "inferswarm.issue110.last-stage-final-report/1",
                    "plan_digest": plan.get("digest"),
                    "producer_freetoken_sha": producer["commit"],
                    "subject": subject,
                    "producer_check": producer_check,
