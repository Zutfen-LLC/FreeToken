"""#110 v5 chain runner: teacher-forced three-stage RTX 3060 candidate.
...
#76 R4 wire service) with the v5 canonical-prefix contract:
...
0/1/3/7 is unchanged from the #76 harness; the v5 all-8-decision
consumer-logit reducer rule is satisfied by the retained decision-<i>.f32
rows at ALL 8 decisions.

Run from a #110 worktree on inferswarm01 with the #110 last-stage service
...
def _load_reference_case(path: Path, case: dict, subject: dict) -> dict:
    from benchmarks.inferswarm_110 import REFERENCE_CASE_SCHEMA

    reference = json.loads(path.read_text())
    if reference.get("schema") != REFERENCE_CASE_SCHEMA:
        raise SystemExit(f"{path}: not a v5 reference case summary")
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--reference-dir", required=True,
                        help="v5 reference run root (per-case dirs with "
    ...
    try:
        ...
        for case in cases:
            ...
            for step in range(GENERATED_TOKENS):
                # --- v5 teacher forcing: exact reference prefix ----------
                ...
                # every decision is evidence-bearing in v5: capture_step is
