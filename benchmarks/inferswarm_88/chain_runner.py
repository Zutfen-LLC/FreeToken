"""#88 v3 chain runner: teacher-forced three-stage RTX 3060 candidate.
...
Run from a #88 worktree on inferswarm01 with the #88 last-stage service
...
def _load_reference_case(path: Path, case: dict) -> dict:
    ...
    if reference.get("schema") != "inferswarm.issue88.v3-reference-case/1":
        raise SystemExit(f"{path}: not a v3 reference case summary")
...
def main(argv=None) -> int:
    ...
    try:
        ...
        index = {
            "schema": "inferswarm.issue88.v3-chain-run-index/1",
