"""Deterministic short W4 equality: graph-local reference versus compact AB."""
...
from inferswarm_d3.whole_model_correctness import _vm, _delta
...
def command(root: Path, model: str, port: int, placement: str, compact: bool):
    ...
    if compact:
        cmd += ["--inferswarm-experimental-d5-compact-routes",
...
def run(ns, compact: bool):
    label = "compact-ab" if compact else "graph-local"; port = free_port()
    origin = f"http://127.0.0.1:{port}"; started = time.monotonic()
    handle = start_server(command(ns.repo, ns.model, port, ns.placement, compact), origin,
                          str(ns.output.parent / f"d5-correctness-{label}.server.log"),
    ...
    try:
        ...
        d5 = runtime["inferswarm_d5_compact_routes"]
        checks = {"graph_bs1": runtime["runtime"]["cuda_graph_captured_bs"] == [1],
                  ...
                  "d5_state": (d5["enabled"] and d5["graph_active"] and not d5["eager_fallback"]
                               if compact else not d5["enabled"])}
        ...
        before = _vm(handle.proc.pid); observation = stream_generation(origin, body, timeout=900); after = _vm(handle.proc.pid)
        ...
        tokens = [int(x) for x in records[0]["generated_token_ids"]]
        if len(tokens) != OUTPUT_CAP: raise RuntimeError("short W4 token count mismatch")
        if compact:
            live = snapshot["inferswarm_d5_compact_routes"]
            if (not live["ownership"]["selection_arithmetic_exact"] or
                    ...
                    any(live[k] for k in ("fallback_count", "failure_count", "graph_recapture_count", "steady_state_host_sync_count"))):
                raise RuntimeError(f"D5 whole-model counters invalid: {live}")
        return {"arm": label, "startup_s": time.monotonic() - started, "runtime_checks": checks,
                ...
                "snapshot": snapshot["inferswarm_d5_compact_routes"],
                "paging_delta": _delta(before, after), "response_usage": observation["usage"]}
...
def main() -> int:
    ...
    result = {"schema": "inferswarm.d5.whole-model-correctness/1", "output_cap": OUTPUT_CAP,
    ...
    print(json.dumps({"classification": result["classification"], "hashes": [x["token_sha256"] for x in rows]}, indent=2))
